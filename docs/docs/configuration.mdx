...
Although the examples on this page uses the `graphql-codegen-factories/schema` plugin, the options documented are also compatible with `graphql-codegen-factories/operations`.
